...
func TestActionCRUD(t *testing.T) {
	...
	// Create and delete
	aCreatedIDs := testActionCreate(t, z, 1, 7)
	...
	aCreatedIDs, _, err := z.ActionCreate([]ActionObject{
		{
			Name:        testActionName,
			Eventsource: 0,
			Status:      int(ActionStatusEnabled),
			...
			Filter: ActionFilterObject{
				...
				Conditions: []ActionFilterConditionObject{
					...
					{
						ConditionType: ActionFilterConditionTypeHostGroup,
			...
			Operations: []ActionOperationObject{
				{
					...
					OpConditions: []ActionOperationConditionObject{
						{
							ConditionType: ActionOperationConditionEventAcknowledged,
